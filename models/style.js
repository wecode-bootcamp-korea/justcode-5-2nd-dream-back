...
async function getStyles(keyword) {
  const styles =
    await prisma.$queryRawUnsafe(`SELECT user_name, user_image, style.id AS style_id, style.content, style.image_list, style.comment_num, IFNULL(style_like.like_num,0) AS like_num,
    ...
    FROM style JOIN style_image ON style.id = style_image.style_id LEFT JOIN user ON style.user_id = user.id GROUP BY style.id) style 
    LEFT JOIN 
...
module.exports = { getStyles, addStyleLike, deleteStyleLike };
